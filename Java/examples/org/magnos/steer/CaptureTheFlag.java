...
public class CaptureTheFlag extends SteerBasicExample
{
    ...
    public static final int OBSTACLE_COUNT = 0;
	...
	{
	    playerToMouse = new SteerTo<Vec2>( 1000, mouse );
	...
	public void initializeTeam( Team my, Team their )
	{
	    ...
	    Steer<Vec2> steerCapturerGuard      = new SteerSeparation<Vec2>( PLAYER_ACC_MAX, database, 100, my.flagCapturer, QUERY_MAX, null, Vec2.FACTORY);
	    Steer<Vec2> steerCapturer           = new SteerTo<Vec2>( PLAYER_ACC_MAX, new TargetInterpose<Vec2>( targetCapturer, my.targetCapturer, 0.5f, Vec2.FACTORY ) );
        
	    Steer<Vec2> attackCapturer          = new SteerTo<Vec2>( PLAYER_ACC_MAX, targetOpponentCapturer );
	    Steer<Vec2> guardCapturer           = new SteerSet<Vec2>( steerCapturerGuard, steerCapturer );
	    Steer<Vec2> stayAwayFromOpponent    = new SteerSeparation<Vec2>( PLAYER_ACC_MAX, database, 50, their.flagPlayer, QUERY_MAX, filter, Vec2.FACTORY );
	    Steer<Vec2> freePrisoners           = new SteerTo<Vec2>( PLAYER_ACC_MAX, their.prison );
	    Steer<Vec2> towardsBase             = new SteerTo<Vec2>( PLAYER_ACC_MAX, my.base );
	    Steer<Vec2> attack                  = new SteerTo<Vec2>( PLAYER_ACC_MAX, targetWeakest );
	    Steer<Vec2> towardsOpponentsFlag    = new SteerTo<Vec2>( PLAYER_ACC_MAX, their.base );
	    
	    Steer<Vec2> separation              = new SteerSeparation<Vec2>( PLAYER_ACC_MAX, database, PLAYER_RADIUS, my.flagPlayer, QUERY_MAX, filter, Vec2.FACTORY );
	    Steer<Vec2> stayInside              = new SteerAvoidObstacles<Vec2>( PLAYER_ACC_MAX, database, 40, 20, GROUP_OBSTACLES, 4, Vec2.FACTORY );
	    ...
	    for (int i = 0; i < DEFENSE_SIZE; i++)
	    {
	        ...
	        def.sprite = new SteerSprite( my.color, PLAYER_RADIUS, PLAYER_VEL_MAX / 2, 
	            new SteerSet<Vec2>( PLAYER_ACC_MAX / 3 - 1, // new boolean[] {false, false, true, false, true, true, },
	                ...
	                new SteerSeparation<Vec2>( PLAYER_ACC_MAX / 3, database, 100, my.flagCapturer, QUERY_MAX, null, Vec2.FACTORY ),
	                new SteerTo<Vec2>( PLAYER_ACC_MAX / 3, new TargetLocal<Vec2>( defend, DEFENSE_CIRCLE_RADIUS_MIN, Float.MAX_VALUE ) ),
	                new SteerTo<Vec2>( PLAYER_ACC_MAX / 3, targetWeakestDefense ),
	                new SteerDrive<Vec2>( PLAYER_ACC_MAX / 3, 0, 0, 300 )
	            )
	        );
