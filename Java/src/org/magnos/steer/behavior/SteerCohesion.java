...
public class SteerCohesion<V extends Vec<V>> extends AbstractSteerSpatial<V>
{
    ...
    public SteerCohesion( SpatialDatabase<V> space, float query, long groups, int max, Filter<V, SpatialEntity<V>> filter, V template )
    {
        this( space, query, groups, max, filter, DEFAULT_SHARED, template );
    }

    public SteerCohesion( SpatialDatabase<V> space, float query, long groups, int max, Filter<V, SpatialEntity<V>> filter, boolean shared, V template )
    {
