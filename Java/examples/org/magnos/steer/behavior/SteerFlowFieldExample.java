...
public class SteerFlowFieldExample extends SteerBasicExample
{
	...
	public void start( Scene scene )
	{
		...
		newSprite( Color.blue, 15, 300,  
			flow = new SteerFlowField2( 1000, 50.0f, new Vec2( 0, 0 ), new Vec2( width, height ), field )
		);
	}
	...
	private Vec2[][] generateSineField(int rows, int columns, float xradians, float xoffset, float yradians, float yoffset)
	{
		...
		for (int y = 0; y < rows; y++)
		{
			...
			{
				Vec2 v = new Vec2();
				v.x = (float)Math.sin( y * yradians + yoffset );
				v.y = (float)Math.sin( x * xradians + xoffset );
				v.normali();
				field[y][x] = v;
			}
		}
